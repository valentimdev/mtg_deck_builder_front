    ...
    return (
      <div className="flex items-center justify-center h-full bg-[#2a2b2f]">
        <div className="text-center">
          ...
          <p className="mt-4  text-white">Carregando cartas...</p>
        </div>
      </div>
    );
  }
            ...
            >
              {card.image_uris?.normal ? (
                <div className="relative">
                  ...
                  />
                  {deckItem.quantity > 1 && (
                    <div className="absolute top-2 right-2 bg-(--text-dark) text-white rounded-full w-8 h-8 flex items-center justify-center font-bold text-sm shadow-lg">
                      {deckItem.quantity}
                    </div>
                  )}
                </div>
                ...
                <div className="w-full aspect-63/88 rounded-lg bg-[#3a3b3f] border-2 border-gray-600 flex items-center justify-center p-4">
                  <div className="text-center">
                    ...
                    {deckItem.quantity > 1 && (
                      <p className="text-(--text-dark) font-bold mt-2">x{deckItem.quantity}</p>
                    )}
                  </div>
                </div>
              )}
